...
InstallMethod( KaroubiEnvelope,
               ...
               function( category )
                   ...
                   structure_record := rec( underlying_category := category, category_with_attributes := karoubi_envelope );
                   ...
                   if CurrentOperationWeight(  category_weight_list, "DirectSum" ) < infinity then
                       AddDirectSum( karoubi_envelope,
                                   function ( obj_list )
                                       ...
                                       return KaroubiObject( DirectSumFunctorial( List( obj_list, o -> Idempotent( o ) ) ) );
                                   end);
                   fi;
                   ...
                   if CurrentOperationWeight( category_weight_list, "TensorUnit" ) < infinity then
                       AddTensorUnit( karoubi_envelope,
                                    function()
                                        return KaroubiObject( IdentityMorphism( TensorUnit( category ) ) );
                                    end);
                   fi;
                   ...
                   if CurrentOperationWeight( category_weight_list, "TensorProductOnObjects" ) < infinity then
                       AddTensorProductOnObjects( karoubi_envelope,
                                                function( a, b )
                                                    return KaroubiObject( TensorProductOnMorphisms( Idempotent( a ), Idempotent( b ) ) );
                                                end);
                   fi;

                   if CurrentOperationWeight( category_weight_list, "DualOnObjects" ) < infinity then
                       AddTensorProductOnObjects( karoubi_envelope,
                                                function( a )
                                                    return KaroubiObject( DualOnMorphisms( Idempotent( a ) ) );
                                                end);
                   fi;
